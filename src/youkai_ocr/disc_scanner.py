...
_PANEL_BBOX = (
    _PANEL_ORIGIN[0],
    _PANEL_ORIGIN[1],
    _PANEL_ORIGIN[0] + _PANEL_W,
    _PANEL_ORIGIN[1] + _PANEL_H,
)
...
def parse_slot_from_panel(panel: Image.Image, recognizer: TextRecognizer) -> int | None:
...
def _extract_disc(
    ...
    for i, (name_rel, val_rel) in enumerate(_SUBSTAT_RELS):
        ...
        if not name_text or name_text in {"-", "--"}:
            ...
            name_big = name_crop.resize((name_crop.width * 2, name_crop.height * 2), Image.LANCZOS)
...
def scan_discs(
    ...
    on_first_item: callable | None = None,
    ...
    abort: list[BaseException | None] = [None]
    _SENTINEL = object()
    ...
    def worker() -> None:
        while True:
            ...
            try:
                disc, conf = _extract_disc(frame, calib, cx, cy, recognizer, archive_dir, cell_idx)
            ...
            with lock:
                ...
                n = done[0]
            print(
                f"\r  read {n}{'/' + str(total_discs) if total_discs else ''} discs…",
                end="",
                flush=True,
            )
            if on_item is not None:
    ...
    workers = [Thread(target=worker, name=f"ocr-{i}", daemon=True) for i in range(n_workers)]
    for w in workers:
        w.start()
...
def _equip_detect_rarity(frame: Image.Image, calib: CalibrationResult) -> tuple[int, int]:
    ...
    for y0 in _EQUIP_RARITY_PROBE_YS:
        crop = _crop(
            ...
            calib,
            (
                _EQUIP_RARITY_ICON_X0,
                y0 + _EQUIP_RARITY_ICON_DY0,
                _EQUIP_RARITY_ICON_X1,
                y0 + _EQUIP_RARITY_ICON_DY1,
            ),
        )
        try:
...
def scan_equipped_disc_frame(
    ...
    level_crop = _crop(
        ...
        calib,
        (
            _EQUIP_RARITY_ICON_X1 + 5,
            rarity_y,
            _EQUIP_X1,
            rarity_y + _EQUIP_RARITY_H,
        ),
    )
    level_text = recognizer.read_line(level_crop, "white_text_on_dark")
    level = parse_level(level_text)
    ...
    block_crop = _crop(
        ...
        (
            _EQUIP_X0,
            rarity_y + _EQUIP_BLOCK_DY0,
            _EQUIP_X1,
            rarity_y + _EQUIP_BLOCK_DY1,
        ),
    )
