...
impl Wish {
    ...
    async fn get_data_dir(&self) -> Result<PathBuf> {
        ...
        let game_data_re = Regex::new(r"(?m).:[/\\].+(ZenlessZoneZero_Data)")?;
    ...
    async fn handle_web_cache_dir_update(&mut self) -> Result<()> {
        ...
        let url_re = Regex::new(r"(https://[a-zA-Z0-9.-]+getGachaLog\?[a-zA-Z0-9.&_=-]+)")?;
...
fn output_log_path() -> Result<PathBuf> {
    ...
    output_log_path.push("AppData/LocalLow/miHoYo/ZenlessZoneZero/Player.log");
