...
use crate::zod::{self, ZodAgent, ZodDisc, ZodExport, ZodSubstat, ZodWEngine};
...
impl PlayerData {
    ...
    pub fn export_zenless_optimizer(&self, settings: &ExportSettings) -> Result<String> {
        let mut zod = ZodExport {
            format: "GOOD".to_string(),
            version: 1,
            source: "Youkai".to_string(),
            characters: Vec::new(),
            discs: Vec::new(),
            weapons: Vec::new(),
        };
        ...
        if settings.include_characters {
            zod.characters = self.export_zenless_optimizer_agents(settings);
        }

        if settings.include_artifacts {
            zod.discs = self.export_zenless_optimizer_discs(settings);
        }

        if settings.include_weapons {
            zod.weapons = self.export_zenless_optimizer_wengines(settings);
        }

        let json = serde_json::to_string(&zod)?;
        tracing::trace!("{json}");
        Ok(json)
    }

    pub fn export_zenless_optimizer_agents(&self, settings: &ExportSettings) -> Vec<ZodAgent> {
        self.characters
            .iter()
    ...
    pub fn export_zenless_optimizer_discs(&self, settings: &ExportSettings) -> Vec<ZodDisc> {
        self.items
            .iter()
            .filter_map(|item| {
                if !item.has_equip() {
                ...
                let location = self
                    ...
                    .and_then(|id| {
                        self.game_data
                            .get_character(*id)
                            .ok()
                            .map(|location| zod::to_zod_key(location).to_string())
                    })
                    .unwrap_or_default();
                ...
                let substats = substats
                    .into_iter()
                    .map(|(property, (value, _initial_value))| ZodSubstat {
                ...
                Some(ZodDisc {
                    set_key: zod::to_zod_key(&artifact_data.set),
    ...
    pub fn export_zenless_optimizer_wengines(&self, settings: &ExportSettings) -> Vec<ZodWEngine> {
        self.items
            .iter()
            .filter_map(|item| {
                if !item.has_equip() {
                ...
                Some(ZodWEngine {
                    key: zod::to_zod_key(&weapon_data.name),
                    level,
                    ascension,
                    refinement,
                    location,
