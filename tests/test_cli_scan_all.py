...
def test_check_disc_screen_raises_when_count_none():
    ...
    with (
        patch("youkai_ocr.disc_scanner.read_disc_count", return_value=None),
...
def test_check_disc_screen_passes_when_count_returned():
    ...
    calib = _identity_calib()
    with (
        patch("youkai_ocr.disc_scanner.read_disc_count", return_value=42),
        patch("youkai_ocr.recognize.make_recognizer", return_value=MagicMock()),
    ):
...
def test_check_engine_screen_warns_when_count_none(capsys):
    ...
    calib = _identity_calib()
    with (
        patch("youkai_ocr.wengine_scanner.read_engine_count", return_value=None),
        patch("youkai_ocr.recognize.make_recognizer", return_value=MagicMock()),
    ):
...
def test_check_engine_screen_passes_when_count_returned(capsys):
    ...
    calib = _identity_calib()
    with (
        patch("youkai_ocr.wengine_scanner.read_engine_count", return_value=5),
        patch("youkai_ocr.recognize.make_recognizer", return_value=MagicMock()),
    ):
...
def test_scan_all_writes_log_and_results_json(tmp_path):
    ...
    assert run_dir.name.startswith("live_"), (
        f"run dir should start with 'live_', got {run_dir.name}"
    )
...
def test_scan_all_resume_skips_disc_and_engine_phases(tmp_path):
    ...
    disc = _sample_disc()
    engine = _sample_engine()
    (resume_dir / "discs.json").write_text(json.dumps([disc.to_dict()]), encoding="utf-8")
    (resume_dir / "engines.json").write_text(json.dumps([engine.to_dict()]), encoding="utf-8")
...
def test_scan_all_issues_json_carries_disc_repairs(tmp_path):
    ...
    assert (
        "substat[0]: {'key': 'def', 'value': 44.0} -> {'key': 'def_', 'value': 14.4}" in review_text
    )
...
def test_auto_nav_raises_if_not_on_main_menu(tmp_path):
    ...
    with (
        patch("youkai_ocr.capture.calibrate_window", return_value=(calib, lambda: dark)),
...
def test_coverage_no_warning_when_counts_match(tmp_path, capsys):
    ...
    with (
        ...
        patch("youkai_ocr.agent_scanner.scan_agents", return_value=([agent], [], [], [])),
        patch(
            "youkai_ocr.agent_scanner.detect_owned_agent_cells", return_value=_make_owned_cells(1)
        ),
        patch("youkai_ocr.cli._preflight_frame", return_value=_fake_frame()),
        patch("youkai_ocr.cli._countdown"),
        patch("youkai_ocr.cli._check_disc_screen"),
...
def test_coverage_warning_when_fewer_agents_scanned(tmp_path, capsys):
    ...
    with (
        ...
        patch("youkai_ocr.agent_scanner.scan_agents", return_value=([agent], [], [], [])),
        patch(
            "youkai_ocr.agent_scanner.detect_owned_agent_cells", return_value=_make_owned_cells(3)
        ),
        patch("youkai_ocr.cli._preflight_frame", return_value=_fake_frame()),
        patch("youkai_ocr.cli._countdown"),
        patch("youkai_ocr.cli._check_disc_screen"),
...
def test_coverage_skipped_when_agents_phase_skipped(tmp_path):
    ...
    with (
        patch("youkai_ocr.capture.calibrate_window", return_value=(calib, MagicMock())),
        patch("youkai_ocr.disc_scanner.scan_discs", return_value=([disc], [])),
        patch("youkai_ocr.wengine_scanner.scan_engines"),
...
def test_porcelain_stdout_is_pure_jsonl(tmp_path):
    ...
    assert len(terminal) == 1, (
        f"expected exactly one terminal event, got: {[r['event'] for r in rows]}"
    )
...
def test_porcelain_error_event_on_screen_assert_error(tmp_path):
    ...
    with patch("youkai_ocr.capture.calibrate_window", side_effect=_raise), patch("sys.stdout", buf):
...
def test_preflight_frame_dark_interactive_calls_input():
    """In interactive mode a dark frame calls input();
    monkeypatching it to raise proves the call."""
    calib = _identity_calib()
    ...
    with (
        patch("youkai_ocr.capture.check_color_hygiene"),
        patch(
            "builtins.input", side_effect=AssertionError("input must not be called in porcelain")
        ),
    ):
        with pytest.raises(AssertionError, match="input must not be called"):
            _preflight_frame(
                _capture, calib, archive_dir=None, phase="discs", interactive=True, emitter=None
            )


def test_preflight_frame_dark_noninteractive_does_not_call_input():
    """In non-interactive mode, builtins.input must never be called
    even in the dark-frame branch."""
    calib = _identity_calib()
    frame = _dark_frame(brightness=10)
    emitter = _FakeEmitter()
...
def test_preflight_frame_black_raises_regardless_of_interactive():
    ...
    with patch("youkai_ocr.capture.check_color_hygiene"):
        with pytest.raises(RuntimeError, match="nearly black"):
            _preflight_frame(
                _capture, calib, archive_dir=None, phase="discs", interactive=False, emitter=emitter
            )
...
def _engine(key, level=60, location="") -> ZodWEngine:
    return ZodWEngine(
        key=key, level=level, ascension=5, refinement=1, location=location, lock=False
    )
