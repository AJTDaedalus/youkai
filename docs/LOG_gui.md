...
**Outcome — reverted to reference_18.** All three "make it bigger" approaches (ScrollArea, inner_size 0.6, inner_size 0.6 + zoom 1.2) broke the layout on Windows: empty left terminal, missing EXECUTE button, params box overflowing the frame. Root cause not pinned down — the left column / params panels use fragile absolute geometry (`allocate_ui_at_rect` + `set_clip_rect(intersect max_rect)` + `set_height(available-6)` + `bottom_up`) that misbehaves when the panel point-size changes, in a way that contradicts the naive height math. Could not reproduce headless (no GPU; Xvfb+llvmpipe renders blank), so changes were unverifiable.
